...
func NewBookArgs() *Book {
	...
	flag.StringVar(&book.Cover, "cover", "cover.jpg", "封面图片可为: 本地图片, 和orly。 设置为orly时生成orly风格的封面, 需要连接网络。")
	...
	flag.StringVar(&book.Format, "format", GetEnv("KAF_CLI_FORMAT", "epub"), "书籍格式: all、epub、mobi、azw3。环境变量KAF_CLI_FORMAT可修改默认值")
...
func (book *Book) SetDefault() {
	...
	book.Cover = defaultString(book.Cover, "cover.jpg")
	book.Bottom = defaultString(book.Bottom, "1em")
	book.Lang = defaultString(book.Lang, GetEnv("KAF_CLI_LANG", "zh"))
	book.Format = defaultString(book.Format, GetEnv("KAF_CLI_FORMAT", "epub"))
}
